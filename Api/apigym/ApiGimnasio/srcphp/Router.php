...
class Router
{
    ...
    public static function route($route, $path_to_include)
    {
        ...
        // Inicializar $parameters como un array vacío
        $parameters = [];
        ...
        if ($route_parts[0] == '' && count($request_url_parts) == 0) {
            ...
            if (is_array($path_to_include)) {
                $controller = new $path_to_include[0]();
                call_user_func_array([$controller, $path_to_include[1]], $parameters);
                exit();
            } else {
                include_once __DIR__ . "/$path_to_include";
                exit();
        ...
        if (is_array($callback)) {
            try {
                $i = new $callback[0]();
                // Verificar si $parameters es un array antes de usarlo
                call_user_func_array([$i, $callback[1]], is_array($parameters) ? $parameters : []);
                exit();
            } catch (\Exception $e) {
                echo json_encode([
                    "error" => $e->getMessage()
